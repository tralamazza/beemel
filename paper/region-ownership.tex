...
\maketitle
\begin{center}
\small\emph{Preprint; comments welcome.}\\[3pt]
\small DOI:~\href{https://doi.org/10.5281/zenodo.20878733}{10.5281/zenodo.20878733}
\end{center}
\vspace{0.5em}
...
permissions --- under a compact declaration schema that held up unchanged
against three structurally different DMA architectures.
...
(\S\ref{sec:eval}): every check is traced to a concrete bug provoked on hardware;
the checks are corroborated against independently-documented vendor hazards and
third-party driver bugs the language was not co-designed with; the model is
validated on three boards; and we report the cost (annotations and generated
code) and the abstract-interpretation encodings required to discharge the DMA
...
\texttt{DST} is shared by the thread, an ISR, and the MDMA engine --- both by CPU
contexts and by an agent --- so every access outside the window is
...
own bus master). None of the three forced a seventh question into the contract; the
schema is closed only modulo the acknowledged future refinements of \S\ref{sec:limits}
...
channel's own \texttt{completes\_by} flag (Table~\ref{tab:target}) --- a
conservative, non-flow-sensitive check. A plain view over
...
bus         = axi: 0x30000000..0x30008000,   # covers dma_pool
              ahbs: 0x20000000..0x20020000    # covers dtcm
...
\end{lstlisting}
\caption{A target-file fragment (the physics layer). No key takes a raw address:
...
were built against. \S\ref{sec:eval-cost} then measures what the checks cost ---
...
\begin{tabular}{@{}p{3.1cm}p{2.7cm}p{3.1cm}p{5.9cm}@{}}
\toprule
\textbf{Documented hazard (chip)} & \textbf{Attested by} & \textbf{Caught today by} & \textbf{BML, at compile time} \\
\midrule
DMA buffer in DTCM (STM32H7) & AN4839~\cite{an4839}; ST community & nothing --- the board silently never transmits & rejected: \emph{``\ldots mem \texttt{dtcm} \ldots agent \texttt{eth} cannot reach''} \\[6pt]
DMA buffer in CCM RAM (STM32F4) & ST community; NuttX \texttt{CCMEXCLUDE} & runtime AHB / hard fault & rejected: \emph{``\ldots mem \texttt{ccm} \ldots agent \texttt{dma2} cannot reach''} \\[6pt]
EasyDMA buffer in flash (nRF52) & Nordic spec~\cite{easydma}; \texttt{nrf-hal}~\#37 & hard fault or silent; embassy's run-time \texttt{BufferNotInRAM} & rejected: \emph{``\ldots mem \texttt{flash} \ldots agent \texttt{easydma} cannot reach''} \\[6pt]
Cacheable DMA buffer, D-cache on (STM32H7, SAME70) & AN4839~\cite{an4839}; Zephyr~\#36471~\cite{zephyrdma} & silent corruption (``works with cache off'') & rejected: \emph{``\ldots cache views diverge''}; \texttt{cacheable=false} then generates the non-cacheable MPU region \\
\bottomrule
\end{tabular}
\caption{Independently-documented, multi-vendor DMA hazards, each reconstructed
in BML and rejected by the compiler. One region reach check (rows 1--3) catches
...
Two observations. First, a single check --- the region reach test --- catches the
...
The honest scope of \S\ref{sec:eval-catalog} carries over here too.
...
existence proofs that the model is self-consistent and runs end to end; on their
own they do not isolate the \emph{value} of the unification over two separate
mechanisms (\S\ref{sec:limits}) --- though the H723 bring-up below does exercise
the full claim/reclaim composition of \S\ref{sec:ownership} on silicon.
...
\subsection{Cost: annotations and code size}
\label{sec:eval-cost}

All three measurements come from running the compiler; none needs hardware.

\paragraph{Annotation burden.} A program declares chip physics once per chip ---
the shipped \texttt{stm32h723.target} is 51 lines of declared physics, reused by
every H723 program --- and adds a small project policy (the H723 example: an
\texttt{include} plus three \texttt{[region.*]} blocks). In source, the model's
annotations
(\texttt{owns}, \texttt{in}, \texttt{@shared}, plus the descriptor-level
\texttt{addr in} and \texttt{@extent}) are a small fraction of the code: 26 of 701 lines (3.7\%) in the Ethernet driver,
and 4--10 lines in the smaller examples (the 51-line MDMA demo is denser at
${\sim}20\%$, being almost entirely DMA orchestration).

\paragraph{Generated code.} The compile-time checks --- reach, ownership,
completion guard --- emit \emph{no} code; they are analysis. The runtime additions
are \emph{derived} and small. The non-cacheable MPU region costs 64 bytes of
\texttt{.text} (198 vs.\ 134 with the region toggled on and off, isolating the
five PMSAv7 register writes), programmed once at reset. Each handoff adds one
\texttt{dsb}; each \texttt{claim} window a \texttt{BASEPRI} raise and restore.
These are the instructions a \emph{correct} hand-written driver must already emit
--- BML's overhead relative to correct code is essentially zero; relative to the
buggy code of \S\ref{sec:eval-catalog} it is the cost of being correct.

\paragraph{Mechanization.} BML generates that 64-byte non-cacheable region, and
the cache-line alignment of buffers placed in it, from a single declaration
(\texttt{cacheable = false}). The hand-written equivalent --- the pattern in ST's
examples, Zephyr, and \texttt{stm32-eth} --- is an MPU configuration routine (the
CMSIS \texttt{MPU\_Config} idiom, ${\sim}20$ lines per region~\cite{an4839}), a
dedicated linker section, per-buffer alignment attributes, and, unless the region
is non-cacheable, explicit cache maintenance at every transfer. One declaration
replaces all of it.

\paragraph{Caveats.} The annotation figures are raw content-line counts, crude by
nature and reported as-is rather than as a spun ratio; the mechanization
comparison's manual-side figure is a representative count for the CMSIS idiom, not
a measurement of one driver. ``Zero overhead versus correct code'' compares to a
correctly-written driver, not to no driver at all.
...
device interfaces type-safe but excluded DMA from that safety: ``the only unsafe
aspect of the driver-device interface is DMA,'' since a misbehaving driver ``can
program a DMA-capable device to overwrite any part of memory.''
Ivory and Tower~\cite{ivory} give a memory-safe embedded DSL whose ``regions''
are allocation/effect regions in the Tofte--Talpin sense~\cite{toftetalpin}, not
hardware-shared
...
\newblock Stack-based scheduling of real-time processes.
\newblock \emph{Real-Time Systems}, 3(1):67--100, 1991.
...
\newblock In \emph{Proc. EMSOFT}, pages 255--264, 2008.
...
\bibitem{pancake} J.~Zhao et al.
\newblock Verifying device drivers with Pancake.
...
\newblock PISTIS: Trusted computing architecture for low-end embedded systems.
\newblock In \emph{Proc. USENIX Security Symposium}, pages 3843--3860, 2022.

\bibitem{singularity} G.~Hunt et al.
\newblock An overview of the Singularity project.
\newblock Microsoft Research Technical Report MSR-TR-2005-135, 2005.

\bibitem{ivory} P.~C. Hickey, L.~Pike, T.~Elliott, J.~Bielman, and J.~Launchbury.
\newblock Building embedded systems with embedded DSLs.
\newblock In \emph{Proc. ICFP}, pages 3--9, 2014.

\bibitem{toftetalpin} M.~Tofte and J.-P. Talpin.
...
\newblock In \emph{Proc. SOSP}, pages 234--251, 2017.
...
\newblock In \emph{Proc. SEFM}, pages 271--277, 2014.
...
\newblock In \emph{Proc. PLDI}, pages 158--174, 2021.
...
\newblock In \emph{Proc. ACM Program. Lang.} 7(POPL), pages 1--32, 2023.

\bibitem{cogent} S.~Amani et al.
\newblock Cogent: Verifying high-assurance file system implementations.
\newblock In \emph{Proc. ASPLOS}, pages 175--188, 2016.
