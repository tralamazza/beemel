            ...
            for &t in TYPES {
                ...
                if width(t) <= 32 {
                    ...
                    body.push_str(&format!(
                        "    semihost.expect_u32(({src}) as u32, {expected});\n"
                    ));
                } else {
                    ...
                    body.push_str(&format!("    semihost.expect_u64({lhs}, {bits}u64);\n"));
                }
            }
        }
        let program = format!(
        ...
        for _ in 0..60 {
            for t in [Ty::F32, Ty::F64] {
                ...
                if is32 {
                    ...
                    body.push_str(&format!("    semihost.expect_u32(*pf{k}, {bits});\n"));
                } else {
                    body.push_str(&format!("    var vf{k}: f64 = {src};\n"));
                    body.push_str(&format!("    var pf{k}: *u64 = (&vf{k}) as *u64;\n"));
                    body.push_str(&format!("    semihost.expect_u64(*pf{k}, {bits}u64);\n"));
                }
                k += 1;
            }
        }
