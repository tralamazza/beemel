...
| Atomic operations | `volatile` + `std::atomic` | `@shared(...)` → masked critical sections (BASEPRI raised to the derived ceiling on ARMv7(E)-M, so unrelated higher-priority ISRs keep running; `cpsid`/`cpsie` on ARMv6-M); `claim` for multi-access windows |
