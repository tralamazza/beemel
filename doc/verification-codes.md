...
| V130 | sio / uio | Signed/unsigned integer overflow. Signed-typed (iN) add/sub/mul/neg carry `nsw` in the verify IR (only there -- runtime IR stays flag-free), so IKOS checks them as SIGNED overflow with branch narrowing; unsigned ops are checked unsigned. Not reported on lines covered by a wrapping-arithmetic expression (`+%`/`-%`/`*%`): wrap there is declared intent |
