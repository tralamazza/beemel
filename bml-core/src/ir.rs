...
use crate::ast::{self, BitSpec, Expr, LValue, Program, Stmt, StorageAnnotation};
...
/// Compute (mask, shift) for extracting a bit field from a u32 register value.
fn bit_mask_shift(bits: &BitSpec) -> (u32, u32) {
    match bits {
        BitSpec::Single(n) => {
            let n = *n as u32;
            (1 << n, n)
...
/// Bit-band regions for Cortex-M3/M4.
...
/// If the given register address and single-bit spec are eligible for bit-band
/// access, returns the alias address. Otherwise returns None.
fn bitband_alias(addr: u64, bits: &BitSpec) -> Option<u64> {
    let bit = match bits {
...
impl IrEmitter {
    ...
    fn emit_module_header(&mut self) {
        ...
        self.line(&format!("target triple = \"{}\"", self.target_triple));
        self.line("target datalayout = \"e-m:e-p:32:32-i64:64-v128:64:128-a:0:32-n32-S64\"");
        if self.debug {
            self.line("");
    ...
    fn emit_function(&mut self, fn_def: &ast::FnDef, symbols: &SymbolTable) {
        ...
        if tailchain && has_calls {
            self.line("call void asm sideeffect \"push {lr}\", \"\"()");
        }
        ...
        if tailchain {
            if has_calls {
                self.line("call void asm sideeffect \"pop {pc}\", \"\"()");
            } else {
                self.line("call void asm sideeffect \"bx lr\", \"\"()");
            }
        ...
        match stmt {
            Stmt::VarDecl(vd) => {
                ...
                let final_reg = if init_llvm == llvm_ty {
                    ...
                    let reg = self.new_reg();
                    if crate::types::is_float(&init_ty) && crate::types::is_float(&bml_type) {
        ...
        match expr {
            ...
            Expr::Ident((name, _)) => {
                ...
                if let Some(p) = symbols.peripherals.get(name) {
                    let reg = self.new_reg();
                    self.line(&format!("{reg} = add i32 0, {}", p.base_addr));
                    return reg;
                }
            ...
            Expr::Binary(left, op, right) => {
                    ...
                    && *op == BinaryOp::Sub
                {
                    ...
                    self.line(&format!("{left_int} = ptrtoint ptr {left_reg} to i32"));
                    self.line(&format!("{right_int} = ptrtoint ptr {right_reg} to i32"));
                    let diff = self.new_reg();
                    self.line(&format!("{diff} = sub i32 {left_int}, {right_int}"));
                    let reg = self.new_reg();
            ...
            Expr::FieldAccess(base, field) => {
                    ...
                    && let Some(reg) = p.regs.get(&field.0)
                {
                    ...
                    self.line(&format!(
                        "{reg_name} = load volatile i32, ptr inttoptr (i32 {addr} to ptr)"
                    ));
                    return reg_name;
                }
                    ...
                    && let Some(field_def) = reg.fields.get(&field.0)
                {
                    ...
                    if self.has_bitband
                        && let Some(alias) = bitband_alias(addr, &field_def.bit_spec)
                    {
                        let val_reg = self.new_reg();
                        self.line(&format!(
                            "{val_reg} = load volatile i32, ptr inttoptr (i32 {alias} to ptr)"
                        ));
                    ...
                    let val_reg = self.new_reg();
                    self.line(&format!(
                        "{val_reg} = load volatile i32, ptr inttoptr (i32 {addr} to ptr)"
                    ));
                    let (mask, shift) = bit_mask_shift(&field_def.bit_spec);
                ...
                let _base_reg = self.emit_expr(base, symbols, fn_name);
                let reg = self.new_reg();
            ...
            Expr::Index(base, index) => {
                ...
                if crate::types::is_ptr(&base_ty) {
                    ...
                    // Fallback
                    let _base_reg = self.emit_expr(base, symbols, fn_name);
                    let _idx_reg = self.emit_expr(index, symbols, fn_name);
                    let reg = self.new_reg();
    ...
    fn emit_lvalue_ptr(&mut self, expr: &Expr, symbols: &SymbolTable) -> String {
        match expr {
            Expr::Ident((name, _)) => {
                ...
                if let Some(p) = symbols.peripherals.get(name) {
                    let reg = self.new_reg();
                    self.line(&format!("{reg} = inttoptr i32 {} to ptr", p.base_addr));
                    return reg;
                }
            ...
            Expr::FieldAccess(base, field) => {
                    ...
                    && let Some(reg) = p.regs.get(&field.0)
                {
                    let addr = p.base_addr + reg.offset;
                    let reg_name = self.new_reg();
                    self.line(&format!("{reg_name} = inttoptr i32 {addr} to ptr"));
                    return reg_name;
                }
        ...
        match lval {
            ...
            LValue::Field(base, field) => {
                    ...
                    && let Some(reg) = p.regs.get(&field.0)
                {
                    let addr = p.base_addr + reg.offset;
                    self.line(&format!(
                        "store volatile i32 {val_reg}, ptr inttoptr (i32 {addr} to ptr)"
                    ));
                    return val_reg.to_string();
                }
                    ...
                    && let Some(field_def) = reg.fields.get(&field.0)
                {
                        ...
                        && let Some(alias) = bitband_alias(addr, &field_def.bit_spec)
                    {
                        ...
                        self.line(&format!(
                            "store volatile i32 {alias_val}, ptr inttoptr (i32 {alias} to ptr)"
                        ));
                        return alias_val;
                    }
                    // Fallback RMW write
                    ...
                    self.line(&format!(
                        "{old} = load volatile i32, ptr inttoptr (i32 {addr} to ptr)"
                    ));
                    // clear field bits
                    ...
                    // volatile store back
                    self.line(&format!(
                        "store volatile i32 {new_val}, ptr inttoptr (i32 {addr} to ptr)"
                    ));
                    return new_val;
                }
    ...
    fn emit_startup_routine(&mut self, symbols: &SymbolTable) {
        let has_main = symbols.functions.contains_key("main");

        self.line("@_sdata = external global i8");
        self.line("@_edata = external global i8");
        self.line("@_sidata = external global i8");
        self.line("@_sbss = external global i8");
        self.line("@_ebss = external global i8");
        self.line("");

        self.line("define void @reset_handler() #2 {");
        self.indent += 1;

        self.line("entry:");
        self.line("  br label %data_copy_test");
        self.line("");

        // Allocate registers in definition order across blocks:
        // data_copy_test defines: src, dst, dst_int, edata_int, data_done  (5 regs)
        // data_copy_body defines: val, src_next, dst_next                    (3 regs)
        self.line("data_copy_test:");
        let src = self.new_reg();
        let dst = self.new_reg();
        let dst_int = self.new_reg();
        let edata_int = self.new_reg();
        let data_done = self.new_reg();
        let val = self.new_reg();
        let src_next = self.new_reg();
        let dst_next = self.new_reg();
        self.line(&format!(
        ...
        self.line(&format!("{dst_int} = ptrtoint ptr {dst} to i32"));
        self.line(&format!("{edata_int} = ptrtoint ptr @_edata to i32"));
        self.line(&format!("{data_done} = icmp eq i32 {dst_int}, {edata_int}"));
        self.line(&format!(
            "br i1 {data_done}, label %bss_zero_init, label %data_copy_body"
        ));
        self.line("");

        self.line("data_copy_body:");
        self.line(&format!("{val} = load volatile i8, ptr {src}"));
        self.line(&format!("store volatile i8 {val}, ptr {dst}"));
        self.line(&format!("{src_next} = getelementptr i8, ptr {src}, i32 1"));
        self.line(&format!("{dst_next} = getelementptr i8, ptr {dst}, i32 1"));
        self.line("br label %data_copy_test");
        self.line("");

        self.line("bss_zero_init:");
        self.line("  br label %bss_test");
        self.line("");

        // Allocate registers in definition order:
        // bss_test defines: bss_ptr, bss_int, ebss_int, bss_done  (4 regs)
        // bss_body defines: bss_next                                (1 reg)
        self.line("bss_test:");
        let bss_ptr = self.new_reg();
        let bss_int = self.new_reg();
        let ebss_int = self.new_reg();
        let bss_done = self.new_reg();
        let bss_next = self.new_reg();
        self.line(&format!(
            "{bss_ptr} = phi ptr [ @_sbss, %bss_zero_init ], [ {bss_next}, %bss_body ]"
        ));
        self.line(&format!("{bss_int} = ptrtoint ptr {bss_ptr} to i32"));
        self.line(&format!("{ebss_int} = ptrtoint ptr @_ebss to i32"));
        self.line(&format!("{bss_done} = icmp eq i32 {bss_int}, {ebss_int}"));
        ...
        self.line("bss_body:");
        self.line(&format!("store volatile i8 0, ptr {bss_ptr}"));
        self.line(&format!(
        ...
        if has_main {
            self.line("call_main:");
            self.line("  call void @main()");
            self.line("  br label %halt_loop");
            self.line("");
        }
    ...
    fn emit_vector_table(&mut self, program: &Program, symbols: &SymbolTable) {
        // System exception handlers: label → fixed vector slot
        ...
        // Collect ISR functions by optional label from @isr("label")
        ...
        // Determine default handler name
        let default_handler_name = if symbols.functions.contains_key("Default_Handler") {
            "Default_Handler"
        } else {
            "default_handler"
        };
        // Emit auto-generated default handler if user didn't provide one
        if default_handler_name == "default_handler" {
            self.line("define void @default_handler() #1 {");
            self.line("entry:");
            self.line("  br label %halt_loop");
            self.line("halt_loop:");
            self.line("  br label %halt_loop");
        ...
        // Find reset handler: @isr("Reset") then user-defined reset_handler/Reset_Handler,
        // otherwise auto-generate startup routine that calls main.
        ...
        let reset_fn = if let Some(name) = user_reset {
            ...
            self.emit_startup_routine(symbols);
            "reset_handler".to_string()
        };

        // Build 16 system exception entries
        ...
        // Device IRQ area: labeled first (at target-specified slots), then unlabeled
        let max_irq = self.target_interrupts.values().max().copied().unwrap_or(0) as usize;
        let irq_start = 16;
        let irq_count = max_irq + 1; // slots 0..max_irq inclusive
        let total = irq_start + irq_count;
        entries.resize(total, format!("@{default_handler_name}"));

        // Place labeled IRQs at target-specified positions
        for (label, slot) in &self.target_interrupts {
        ...
        // Place unlabeled ISRs in declaration order into empty IRQ slots
        ...
        for (i, entry) in entries.iter().enumerate() {
            ...
            self.line(&format!("  ptr {entry}{comma}"));
        }
        self.line("], section \".vector_table\"");
        self.line("");

        // Stack top symbol
        self.line("@_stack_top = external global i32");
        self.line("");

        // Attributes
        self.line("attributes #0 = { nounwind }");
        self.line("attributes #1 = { nounwind \"interrupt\" }");
        self.line("attributes #2 = { nounwind optnone noinline }");
    }
    ...
    fn new_str_id(&mut self) -> u32 {
        let id = self.str_counter;
        self.str_counter += 1;
        id
    }

    fn alloca(&mut self, ty: &str, name: &str) -> String {
        let n = self.counter;
        self.counter += 1;
