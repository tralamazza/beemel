...
/// Every reserved word in the language, paired with its token kind.
///
/// Single source of truth: the lexer resolves identifiers against this
/// table, the LSP builds its keyword completions from it, and the front-end
/// fuzzer mixes it into the token pool -- a keyword added here propagates to
/// all three (the LSP completion list silently drifted for months when it
/// was a hand-copied array).
pub const KEYWORDS: &[(&str, TokenKind)] = &[
    ...
    ("peripheral_type", TokenKind::PeripheralType),
    ("reg", TokenKind::Reg),
