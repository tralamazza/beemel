...
const TOKENS: &[&str] = &[
    "stride", "u8", "u16", "u32", "u64", "i8", "i16", "i32", "i64", "f32", "f64", "b1", "b8",
    "bool", "@context", "@isr", "@naked", "main", "a", "b", "foo", "(", ")", "{", "}", "[", "]",
    "<", ">", ",", ";", ":", ".", "@", "=", "==", "!=", "+", "-", "*", "/", "%", "&", "|", "^",
    "~", "<<", ">>", "+%", "-%", "*%", "+%=", "&&", "||", "!", "->", "..", "0", "1", "42", "0xFF",
    "1u8", "1u32", "1i32", "1.5f", "0x4000", "\"s\"",
];

fn gen_tokens(rng: &mut Rng) -> String {
    // The pool is TOKENS (operators, literals, idents, types) plus every
    // language keyword straight from the lexer table, so new keywords are
    // fuzzed without anyone remembering to add them here.
    let n = 1 + rng.below(60);
    let mut s = String::new();
    for _ in 0..n {
        let idx = rng.below((TOKENS.len() + bml_core::lexer::KEYWORDS.len()) as u64) as usize;
        let tok = TOKENS
            .get(idx)
            .copied()
            .unwrap_or_else(|| bml_core::lexer::KEYWORDS[idx - TOKENS.len()].0);
        s.push_str(tok);
        s.push(' ');
    }
    s
